...
// @author       EGem
// @match        *
// @icon         https://files.catbox.moe/4gto4k.png
// @grant        unsafeWindow
// @grant        GM_webRequest
// @sandbox      JavaScript
...
// 3.1 - Added ad bypasses with variables
// 4.0 - ~4,000 lines added, now works like ublock
// ==/CHANGELOG==
...
(function() {
    ...
    const mutatistobserver = new MutationObserver((mutations) => {
    ...
    class JSONPath {
                ...
                case this.#QUANTIFIER:
                    {
                        ...
                        if (length < step.min || length > step.max) {
                            return [];
                        }
                        resultset = [];
                        break;
        ...
        #evaluateExpr(step, owner, k) {
            ...
            case '>=':
                return (v >= step.rval) === target;
            case '^=':
                return `${v}`.startsWith(step.rval) === target;
            case '$=':
                return `${v}`.endsWith(step.rval) === target;
            case '*=':
                return `${v}`.includes(step.rval) === target;
            case 're':
                return step.rval.test(`${v}`);
            default:
                break;
            }
            ...
            case 'repl':
                {
                    ...
                    if (this.#compiled.re === undefined) {
                        ...
                        try {
                            this.#compiled.re = rval.regex !== undefined ? new JSONPath.Regex(rval.regex,rval.flags) : new JSONPath.Regex(rval.pattern.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
                        } catch {}
                    }
    ...
    function freezeElementProperty(property='', selector='', pattern='') {
        const safe = safeSelf();
    ...
    function preventCanvas(contextType='') {
        ...
        const proto = mutatistobserver.HTMLCanvasElement.prototype;
